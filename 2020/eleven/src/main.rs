...
fn tick(state: &State) -> State {
    ...
    for row in 0..height {
        for col in 0..width {
            ...
            if subject == Square::Floor || subject == Square::Occupied {
                continue;
            }
            ...
            let occupied_count = {
                ...
                for &(dy, dx) in vectors.into_iter() {
                    let mut x = col;
                    let mut y = row;
                    println!("begin");
                    loop {
                        if dx < 0 {
                            // if were about to underflow
                            if x == 0 {
                                break;
                            }
                            x -= -dx as usize;
...
fn main() {
    ...
    println!("{} {}", initial_state.len(), initial_state[0].len());
