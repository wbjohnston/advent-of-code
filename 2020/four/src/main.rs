...
fn main() {
    ...
    let n_valid = passports.into_iter().filter(validate_passport).count();

    println!("{}", n_valid);
}


fn parse<'a>(s: &'a str) -> Vec<Passport> {
    let re = Regex::new(r#"((?:(\w+):(#?[\d\w]+)\s*))"#).unwrap();
    let mut v = vec![];
    let passports = s.split("\n\n");
    for passport in passports {
        let mut p = HashMap::new();

        for group in re.captures_iter(passport) {
            let key = group.get(2).map(|x| x.as_str().to_string()).unwrap();
            let value = group.get(3).map(|x| x.as_str().to_string()).unwrap();
...
const INPUT: &'static str = r#"hcl:#6b5442 ecl:brn iyr:2019
