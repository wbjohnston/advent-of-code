...
fn main() {
    ...
    let unqiue_questions_per_group: usize = groups
        .map(|group| {
            group
                .map(|member| member.chars().collect::<HashSet<char>>())
                .enumerate()
                .fold(HashSet::new(), |acc: HashSet<char>, (i, questions)| {
                    if i == 0 {
                        questions
                    } else {
                        acc.intersection(&questions).cloned().collect()
                    }
                })
                .len()
        })
    ...
    println!("{}", unqiue_questions_per_group);
}

const INPUT: &'static str = r#"jmqnkzlsfedaptx
usjfkadqwmeyilph
