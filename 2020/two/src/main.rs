const INPUT: &'static str = r#"13-15 c: cqbhncccjsncqcc
2-3 v: zvdvfd
...
fn parse(st: &str) -> Test {
    let mut iter = st.chars();
    let min_st: String = iter.by_ref().take_while(|x| x.is_numeric()).collect();
    let min = min_st.as_str().parse().unwrap();
    let max_st: String = iter.by_ref().take_while(|x| x.is_numeric()).collect();
    let max = max_st.parse().unwrap();
    let c = iter.by_ref().next().unwrap();

    let st = iter.skip(2).collect();

    Test { min, max, c, st }
}

fn is_valid(test: &Test) -> bool {
    // let c_count = test.st.chars().filter(|x| x == &test.c).count();

    // c_count >= test.min && c_count <= test.max
    if let Some(first) = &test.st.chars().nth(test.min - 1) {
        if let Some(second) = &test.st.chars().nth(test.max - 1) {
...
fn main() {
    ...
    let tests: Vec<_> = lines.into_iter().map(parse).collect();

    dbg!(&tests);

    let n_valid = tests.iter().filter(|x| is_valid(x)).count();
